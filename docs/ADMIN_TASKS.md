...
- 输入管理密码（默认 `wwcxrl2026`，可用环境变量 `VITE_ADMIN_PASSWORD` 覆盖后重新部署）。
...
| 🃏 翻牌记忆 | 卡片对数 4 / 6 / 8 |
| 🧩 滑块拼图 | 拼图大小 3×3 / 4×4 |
| 📖 成语挑战 | 需要答对几题 3 / 5 / 8（温柔版：单题 30 秒，答错或跳过会亮出答案后自动进入下一题，不再扣光小心心重来；按得分评级） |
| 🌟 星星记忆 | 轮数 3–5；难度 轻松(6星·慢) / 普通(6星·中) / 挑战(9星·干扰闪光+倒计时+换位)，含生命值与计分评级 |
| 🎯 井字棋·三局两胜 | 先赢几局 1 / 2 / 3，AI 逐局变强，含计分 |
| 🔢 数独 | 难度 6×6 轻松 / 6×6 普通 / 9×9 挑战，含计时与最佳记录 |
| 🔮 2048·小星球 | 合成 2048 或累计 3000 分完成（方向键 / WASD / 屏幕按钮） |
| ⚪ 黑白翻转棋·6×6 | 与月亮下完整盘 6×6 翻转棋即完成 |

游戏进度会保存在本地（localStorage）并同步到云端，完成即点亮签到按钮；玩到一半关掉再打开会继续。

> 小游戏模板在 `src/main.jsx` 的 `MINI_GAMES` 注册表里集中维护（`src/main.jsx:4939` 附近）。
> 以后想加新模板，只需在注册表加一条 `{ id, label, icon, hint, defaults, fields }` 并在 `DailyInteraction` 的 game 分支加一行渲染即可。

### 嵌入小游戏（管理页下拉里也会出现）

- 中文休闲嵌入游戏：鱼了个鱼、人生重开模拟器、五子棋、麻将连台、霓虹叠塔、点球热浪、菜摊敲敲乐，以及沿用中的换装/黄金矿工/砌砖/贪吃蛇。
- 2026-09 已下线：气球天堂、圣诞气球、笑脸微笑、水果馅饼、弹力球、圣诞熊猫（英文老界面且玩法雷同）。**云端老任务若仍引用这些 id，会自动映射到同类型的新游戏**，签到页不会空白，也无需手动编辑旧任务。
...
- **小信箱评论盖楼**：每条主留言下方有独立评论区，评论按顺序显示为「1 楼、2 楼…」；评论同样可切换小琛/小琳身份，可带文字和图片。数据复用 `wwcxrl_messages` 表，用 `parent_id` 标记它属于哪条主留言（`parent_id` 为空即主留言）；删除主留言时会一并清理它下面的评论。
...
- **网站建议箱**：页脚「💡 网站建议」弹窗（在「更新日志」旁边），小琳/小琛可写下对网站建设与体验的文字建议，自动同步到 Supabase（表 `wwcxrl_feedback`）；未连接云端时回退本地存储。每个人只能删除自己的建议。

> 已上线的老库需要先执行一次建表 SQL（见 `supabase_wwcxrl_schema.sql` 末尾「留言板」「网站建议箱」和「更新日志」三段）才能使用留言板、网站建议箱与更新日志的云端同步。
> 评论盖楼还需要对已上线的老库补执行：
>
> ```sql
